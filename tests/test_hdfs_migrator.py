...
LISTING = (_REPO_ROOT / "files" / "sample_hdfs" / "hdfs_listing.txt").read_text()


class TestParseHDFSListing:
    def test_returns_list(self):
        entries = parse_hdfs_listing(LISTING)
    ...
    def test_entry_paths_start_with_slash(self):
        entries = parse_hdfs_listing(LISTING)
        for e in entries:
            assert e.path.startswith("/")

    def test_entry_is_hdfs_entry(self):
        entries = parse_hdfs_listing(LISTING)
        assert all(isinstance(e, HDFSEntry) for e in entries)

    def test_file_path_contains_parquet(self):
        entries = parse_hdfs_listing(LISTING)
        files = [e for e in entries if e.entry_type == "file"]
...
class TestGenerateFSCPScript:
    def setup_method(self):
        self.entries = parse_hdfs_listing(LISTING)

    def test_contains_databricks_fs_cp(self):
        script = generate_fs_cp_script(self.entries, hdfs_host="hdfs://namenode:8020")
        assert "databricks fs cp" in script

    def test_source_contains_hdfs_host(self):
        script = generate_fs_cp_script(self.entries, hdfs_host="hdfs://namenode:8020")
        assert "hdfs://namenode:8020" in script

    def test_destination_has_dbfs_prefix(self):
        script = generate_fs_cp_script(self.entries, hdfs_host="hdfs://namenode:8020")
...
class TestGenerateDbutilsScript:
    ...
    def test_dbutils_fs_cp_present(self):
        script = generate_dbutils_script(self.entries, hdfs_host="hdfs://namenode:8020")
        assert "dbutils.fs.cp" in script

    def test_file_count_in_script(self):
        script = generate_dbutils_script(self.entries, hdfs_host="hdfs://namenode:8020")
